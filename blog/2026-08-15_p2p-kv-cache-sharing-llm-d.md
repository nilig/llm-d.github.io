...
*The EPP selects both the destination pod and the source peer, and passes the decision to the consumer as a header. The routing sidecar turns it into a pull: a ZMQ control exchange settles which blocks the peer holds, NIXL moves those blocks CPU-tier to CPU-tier (the source GPU is never touched), hits load into the consumer's GPU as normal cache hits, and any unmatched blocks recompute - so a partial or failed transfer degrades to today's behavior.*
