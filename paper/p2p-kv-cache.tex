...
\title{\bf Peer-to-Peer KV-Cache Sharing for Disaggregated LLM Serving}
\author{
...
latency at 48K tokens). End to end, adding P2P to the \llmd{} P/D
reference deployment is strictly beneficial at high load: on a
document-Q\&A workload, median TTFT drops $10.3\times$ with $+40\%$
throughput; on agentic session shapes (10--100K-token contexts,
tool-call idle gaps), median TTFT drops $4.8\times$ with $+33\%$
throughput, while tail latency is bounded in both arms by the
irreducible cold prefill. We further show that placement policy, not
...
(\S\ref{sec:crossover}); paired end-to-end comparisons on the \llmd{}
