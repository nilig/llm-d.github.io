...
model, because re-tokenized text diverges from generated IDs. TP-mismatched peers are supported only for non-hybrid-attention
models on the V1 model runner (models defaulting to the V2 runner must
be forced back via an environment variable); hybrid models such as
gpt-oss require matched TP, and the topologies here run matched TP
throughout. Finally, per-peer sessions are established lazily, so the
